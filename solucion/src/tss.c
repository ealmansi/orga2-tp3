...
void tss_inicializar_bandera(unsigned int tarea) {
	...
	tss_banderas[tarea].eip			= TASK_1_CODE_SRC_ADDR + TASK_SIZE * tarea + *((dword_t*)(TASK_1_CODE_SRC_ADDR + TASK_SIZE * (tarea + 1) - 4));
	...
	tss_banderas[tarea].es			= GDT_SEL_DATOS_0;
	tss_banderas[tarea].cs			= GDT_SEL_CODIG_0;
	tss_banderas[tarea].ss			= GDT_SEL_DATOS_0;
	tss_banderas[tarea].ds			= GDT_SEL_DATOS_0;
	tss_banderas[tarea].fs			= GDT_SEL_DATOS_0;	// esto estaba en GDT_SEL_VIDEO_0 pero
	tss_banderas[tarea].gs			= GDT_SEL_DATOS_0;	//  tiraba Invalid TSS exception
